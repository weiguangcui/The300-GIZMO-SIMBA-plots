...
for i, sname in enumerate(snapname):
    ...
    ahfhalo = np.load("/home2/weiguang/Project-300-Clusters/Halo_mass_function_mass-difference/GIZMO/GS_Mass_"+sname+"info.npy")
    ...
    head = readsnap(Xspath+"NewMDCLUSTER_0001/"+sname+'.hdf5', "Header",quiet=True)
    ...
    for n in np.arange(1,325):
        ...
        pos = readsnap(Xspath+cn+"/"+sname+'.hdf5', "Coordinates",quiet=True,ptype=5)
        mas = readsnap(Xspath+cn+"/"+sname+'.hdf5', "BH_Mass", fullmass=True,quiet=True,ptype=5)
        mtree = cKDTree(pos)
        spos = readsnap(Xspath+cn+"/"+sname+'.hdf5', "Coordinates",quiet=True,ptype=4)
        smas = readsnap(Xspath+cn+"/"+sname+'.hdf5', "Masses", fullmass=True,quiet=True,ptype=4)
        svel = readsnap(Xspath+cn+"/"+sname+'.hdf5', "Velocities", quiet=True,ptype=4)/np.sqrt(1+head.Redshift)
        smtree = cKDTree(spos)
...
#                 DX200[j,19]=np.sqrt(np.std(tmpv[:,0])**2+np.std(tmpv[:,1])**2+np.std(tmpv[:,2])**2)/np.sqrt(3) #1d sigma
                smm=np.sum(smas[idlist])
                DX200[j,16]=smm*1e10
                hbv =np.std(np.tile(smas[idlist],(3,1)).T*tmpv,axis=0)/np.mean(smas[idlist])
                DX200[j,19]=np.sqrt(np.sum(hbv**2)/3)
#                 DX200[j,19]=np.sqrt(np.sum((tmpv-hbv)**2*np.tile(smas[idlist],(3,1)).T)/smm)
            ...
            if len(idlist)>0:
                tmpv=svel[idlist]# +head.Time*Hubblez*(spos[idlist]-cc)/head.HubbleParam
#                 DX200[j,20]=np.sqrt(np.std(tmpv[:,0])**2+np.std(tmpv[:,1])**2+np.std(tmpv[:,2])**2)/np.sqrt(3)
                smm=np.sum(smas[idlist])
                DX200[j,17]=smm*1e10
                hbv =np.std(tmpv*np.tile(smas[idlist],(3,1)).T, axis=0)/np.mean(smas[idlist])
                DX200[j,20]=np.sqrt(np.sum(hbv**2)/3)
                
            idlist = smtree.query_ball_point(cc, 0.1*DX200[j,13], workers=4)
            if len(idlist)>0:
                tmpv=svel[idlist]# +head.Time*Hubblez*(spos[idlist]-cc)/head.HubbleParam
                DX200[j,21]=np.sqrt(np.std(tmpv[:,0])**2+np.std(tmpv[:,1])**2+np.std(tmpv[:,2])**2)/np.sqrt(3)
                smm=np.sum(smas[idlist])
                DX200[j,18]=smm*1e10
                hbv =np.std(tmpv*np.tile(smas[idlist],(3,1)).T, axis=0)/np.mean(smas[idlist])
                DX200[j,21]=np.sqrt(np.sum(hbv**2)/3)

    np.save("./data/GIZMO_Mbh_"+sname+"-with_BH-info", DX200) 
